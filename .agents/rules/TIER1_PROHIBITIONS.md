...
**NEVER** run `rm`, `rm -rf`, `del`, or delete any existing project file, source file, config, or directory without explicit user authorization ("yes, delete it").

This prohibition applies to:
- Project files, directories, and documentation
- Cache files (they auto-invalidate — DO NOT manually delete)
- Backup files
- Build artifacts (use the build system's standard clean command)
- Lock files (investigate what holds the lock first)

**Exception**: Temporary scratchpad files created exclusively by the agent in the current session (e.g. within `scratch/` or ephemeral test scripts) MUST be cleaned up by the agent and do not require separate user approval.
